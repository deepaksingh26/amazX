...
const ApplyForLeave=({employeeData})=>{
  console.log(":emp",employeeData);
    ...
    const getLeaveData=async()=>{
       const leaves=await axios.get(`https://amazx-1.onrender.com/leave/${id}`)
    ...
    const SubmitHandler=async(e)=>{
        ...
        const applyforleave=await axios.post("https://amazx-1.onrender.com/leave/"+id,{...leave,status:"pending",employee_id:employeeData.name,name:employeeData.name})
        Swal.fire({
          icon: 'success',
