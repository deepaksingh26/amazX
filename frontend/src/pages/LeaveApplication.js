...
const LeaveApplication=()=>{
    ...
    const getPendingLeaveData=async()=>{
       const penleave= await axios.get("https://amazx-1.onrender.com/pendingleave")
    ...
    const updateLeaveStatus=async(status,id)=>{
        const res=await axios.post(`https://amazx-1.onrender.com/updateleavestatus/${id}`,{status:status})
        Swal.fire({
